...
function getGrantedEffectLabels(unit?: Unit | null): string[] {
  ...
  ;(unit.statusEffects || []).forEach((status) => {
    pushUnique(statusNameMap[status] || status)
  })
  ;(unit.tempBuffs?.statusEffects || []).forEach((status) => {
    const statusLabel = statusNameMap[status] || status
    pushUnique(`一時:${statusLabel}`)
  })
        ...
        <div className="mt-2 border-t border-white/20 pt-1">
          ...
          <div className="flex flex-wrap gap-1">
            {grantedEffectLabels.map((label) => (
              <span key={label} className="px-1.5 py-0.5 rounded bg-black/40 text-[8px] text-cyan-200 leading-none">
                {label}
              </span>
            ))}
          </div>
...
export default function GameBoard(props: GameBoardProps) {
  ...
  // カードタップ → 効果表示
  const onCardTap = (cardDef: CardDefinition, side: 'left' | 'right', unit?: Unit | null) => {
