    ...
    >
      ...
      {unit && (
        <div className="mt-2 pt-2 border-t border-white/10">
          <div className="text-white/90 font-bold text-[10px] mb-1">付与中の効果</div>
          {(() => {
            ...
            // DASTER(前方)系は damageReduction を負で保持しているため、ここでも表示する
            ...
            if (effectDamageIncreaseFromDamageReduction !== 0) {
              const timerSec = 0
              lines.push(`受ける効果ダメージ+${effectDamageIncreaseFromDamageReduction}${timerSec ? `（${timerSec}秒）` : ''}`)
            }
            ...
            if (lines.length === 0) return <p className="text-gray-400 text-[11px]">付与なし</p>
            return (
...
export default function GameBoard(props: GameBoardProps) {
  ...
  // カードタップ → 効果表示
  const onCardTap = (cardDef: CardDefinition, side: 'left' | 'right', unitId?: string) => {
    setDetailCard({ card: cardDef, side, unitId })
  }
                      ...
                      <GameCard
                        ...
                        onClick={() => {
                          if (
                            ...
                            onCardTap(leftCardDef, 'left', leftUnit.id)
                          }
                        }}
                      />
                    </div>
                    ...
                    <GameCard
                      ...
                      onClick={() => {
                        if (
                          ...
                          onCardTap(rightCardDef, 'right', rightUnit.id)
                        }
                      }}
                    />
                    </div>
