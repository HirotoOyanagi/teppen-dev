...
describe('新カードCore効果マッピング - 黒カード', () => {
  const blackCardIds = Array.from({ length: 45 }, (_, index) => {
    const id = index + 136
    return `cor_${String(id).padStart(3, '0')}`
  })

  it('黒カード45枚ぶんのマッピングが揃っている', () => {
    expect(blackCardIds).toHaveLength(45)
    for (const cardId of blackCardIds) {
      expect(newCardEffectFunctions[cardId], `${cardId} is missing`).toBeTruthy()
    }
  })

  it('黒ユニットカードは既存UI前提に合わせて target 指定を持たない', () => {
    for (const cardId of blackCardIds.slice(0, 26)) {
      expect(newCardEffectFunctions[cardId], cardId).not.toContain('target:')
    }
  })

  it('黒アクションカードは対象が必要なカードに target 指定を持つ', () => {
    const targetedBlackActions = [
      'cor_162',
      'cor_163',
      'cor_164',
      'cor_165',
    ...
    for (const cardId of targetedBlackActions) {
      expect(newCardEffectFunctions[cardId], cardId).toContain('target:')
    }
  })
})
